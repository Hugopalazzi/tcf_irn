...
export const getUserExamById = async (supabase: SupabaseClient<Database>, id: string): Promise<{ questionsData: any[], currentQuestionIndex: number, userExamId: string }> => {
	const { data, error: userExamError } = await supabase
		.from('user_exam_questions')
		.select(`id, question:questions(title, choices, audio_url, question_type, correct_answer), user_exam:user_exam_id(current_question_index, question_count)`)
		.eq('user_exam_id', id)
		.order('position');
